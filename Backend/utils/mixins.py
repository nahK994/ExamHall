...
class ModelManagerMixin:
    ...
    def list(self, request):
        serialized_data = self.query_serializer_class(self.get_queryset(), many=True)
    ...
    def retrieve(self, request, pk):
        ...
        if filtered_data:
            serializer = self.query_serializer_class(filtered_data[0])
    ...
    def update(self, request, pk, *args, **kwargs):
        ...
        if serialized_info.is_valid():
            instance = serialized_info.save()
            return Response(self.query_serializer_class(instance).data, status=status.HTTP_200_OK)
